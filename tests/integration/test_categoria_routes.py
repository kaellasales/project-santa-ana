...
def test_atualizar_categoria(client):
    ...
    create_response = client.post("/categorias/", json={"nome": "Nome Antigo"})
    categoria_id = create_response.json()["id"]
    ...
    # Verifica que foi atualizado de verdade
    get_response = client.get(f"/categorias/{categoria_id}")
    assert get_response.json()["nome"] == "Nome Novo"

def test_atualizar_categoria_inexistente(client):
    """Testa atualizar uma categoria que não existe"""
    response = client.put(
        "/categorias/9999",
        json={"nome": "Novo Nome"}
    )
...
def test_deletar_categoria(client):
    ...
    # Arrange - cria categoria
    create_response = client.post("/categorias/", json={"nome": "Remover"})
    categoria_id = create_response.json()["id"]
    
    # Act - deleta
    delete_response = client.delete(f"/categorias/{categoria_id}")
    
    # Assert - verifica que deletou
...
def test_deletar_categoria_inexistente(client):
    """Testa deletar uma categoria que não existe"""
    response = client.delete("/categorias/9999")
